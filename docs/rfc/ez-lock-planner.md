...
**Update:** WP2 has landed. `ez lock --upgrade` runs in the same planner: `lock/up.bend` decides the upgrade from its own questions, `Up.run`, `Pin.upgrade` and `ez/upgrade.bend` are deleted, and `lock_refusal_writes_nothing` holds of every World, so it is tagged EZ-OUT-2. Where the code differs from the sketches below: the upgrade's questions and answers are their own types in `lock/up.bend` (`Up.Ask`, `Up.Answer`), held in a fifth World field, `ups`, and `Asking` carries both kinds; `Inputs` holds the ledger the lock is made from, read (`M.Read`), and `stop`, the upgrade's refusal, and its listing is the committed sources as the upgrade rewrites them (`W.listing.of`); `P.ledger.next` is the model the upgrade renders (`Up.next.model`), while the lock is made from that text read back (`Up.next.read`), so a later plain lock reads what this one did; a checkout at the rev a dependency moves to answers the lock's package question for the new hash (`Up.Laid`), so the tree is not cloned twice, and it is laid under `.ez/lib` when the ledger the upgrade leaves vendors that hash and under `$BEND_LIB` otherwise; the hub is asked about a hash only after a pin advances; `U.judge` gets the real agreement of the pinned commit's checkout, and `False` for a commit that moved, which `judge.move` does not read; the plan's lines a person is told (`Say`, on stdout) come first; and the sources are rewritten once, by the plan, not on every round of questions, which is what kept the demand loop's cost down (see "The planner as built" in [ez-spec.md](ez-spec.md)). The interpreter still exits 1 without writing when git cannot be asked at all (a clone or a `merge-base` that fails), since no effect has run by then.

**Update:** WP7 has landed. EZ-VEN-2 and EZ-VEN-3 are proved; EZ-VEN-1's text layer and its `ez lock --upgrade` half are proved, and the row waits on `ez add` (A2). Where the laws differ from the sketches below: the upgrade's allowlist law is over the `.gitignore` the plan writes, `P.put(w, ".gitignore") == Put{text}`, and holds it to the hashes the upgrade commits (`Up.next.vends`), which `upgrade_vends_the_ledger` ties to the model it renders whenever it writes ez.toml; the unwritten case, a `.gitignore` already in sync, is not yet a law (SPEC.md, "Left to prove"). The source laws take a `.bend` source the World names once (`found(w)` is `pre ++ [Found{at, text}] ++ post`, neither side holding `at`) and the moves an upgrade makes (`ML.moves.ok`), and say what the plan leaves at `at` through `left.at`. Three behavior changes came with it (see ez-spec.md, "Decided behavior changes"): the allowlist names each hash once, a lone blank line in `.gitignore` is kept, and `import <old>` with no `/` after it is no longer rewritten.

**Update:** WP6's first half has landed: EZ-RES-4 and EZ-RES-6 are proved (the laws section below says where the laws differ from the sketches). EZ-RES-5 and EZ-RES-8 are its second half, which has landed too.

**Update:** WP10 has landed the `ez lock` halves of EZ-LED-8, EZ-HASH-3 and EZ-VEN-1, and the three rows are proved. `W.Args` holds the directory the lock runs in, and the planner anchors the questions itself, as `ez add` and `ez fetch` do: a package question carries its source through `P.anchor.src`, and an upgrade question is a `P.Query` of the question as the upgrade looks it up, with the ledger's source, and the question git is asked, `Up.anchor` of it; the interpreter answers the second and records the answer under the first, so nothing in `lock/up.bend` changed. The lock now refuses a package whose files, weighed by their own texts, do not hash to its name, and lays the manifest of those texts, so every tree it lays is named by its manifest (`lock_lays_named`); `ez fetch` already checked this, and a lock that passed a manifest listed out of path order wrote a lock `ez fetch` refused. The two upgrade facts WP7 left are `upgrade_allowlist_kept_in_sync` and `upgrade_unmoved_vends_the_ledger`, proved by a second walk of `Up.next` (`kp_next`) that carries the stop reason: every stage but `Up.done` stops with a reason `Up.halt.why` keeps from being empty, so a lock that does not refuse went through `Up.done`, where an unwritten `.gitignore` is one `I.sync` left as it was and an unwritten ez.toml means no pin moved, so every dependency's fate was to stay.

**Update:** WP5b has landed: EZ-DOC-4 is proved for `ez lock --upgrade` too, relative to EZ-LED-4, and a tool pin with a rev and no narHash, which a second upgrade used to move, is now upgraded in full the first time (see the EZ-DOC-4 section below).

This is the design for phase three of [ez-spec.md](ez-spec.md): converting `ez lock` to a pure planner and a thin interpreter, then proving EZ-DOC-1 through EZ-DOC-5, EZ-RES-4 through EZ-RES-6 and EZ-RES-8, EZ-VEN-1 through EZ-VEN-3, and EZ-HASH-2. It was written from the code at `bec3435`. A spike in `lock/world/` checks the riskiest parts against bend 2.0.25; nothing imports it, so `ez lock` behaves exactly as before. `SPEC.md` does not change until a requirement's law lands; how the work went is summarized in [ez-spec.md](ez-spec.md) under "How we got here".
...
- [x] <!-- REVIEW (resolved): A file path holding `=` is refused by `lockable`. The pinned eztoml v0.1.0 reader cuts a pair line at its first `=` even inside a quoted key (eztoml#24, closed, fixed in 0.2.x), so such a path was written into a lock that read back as another path. EZ-DOC-1 is proved against v0.1.0 with the refusal; a later move to eztoml 0.2.x lifts it. This is a decided behavior change. -->
...
**Update:** WP4 has landed, both steps. It found that a lockable lock could still fail to read back: a file path holding `=` is written as a quoted key, and eztoml v0.1.0's reader cuts the line at that `=` ([eztoml#24](https://github.com/Emerging-Patterns/eztoml/issues/24), closed, fixed in 0.2.x). `ez lock` on a package with a file `a=b.bend` wrote a lock that `ez fetch` read back as the path `a` and refused. `lockable` now also refuses a file path holding `=`, and the planner names the package and the path (`lock/LAWS.bend path_eq_unlockable`, `path_eq_named`); EZ-DOC-1's premise is this `lockable`. Where the proof differs from the sketch: the sections step is `render_is_doc` (the lock's block render is `T.render` of its document, a law WP3's render had lost) and `lock_sections_read_back`, and the generic half of it, that any document whose table names are not empty and whose keys hold no `"`, `=` or newline parses back as itself, is `check/toml.bend sections_read`, shared so EZ-LED-4 can use it. The text step is three tagged laws, `lock_reads_back`, `lock_hub_reads_back` and `lock_tools_read_back`. `canon` is law vocabulary in `lock/LAWS.bend`, not `L.canon`; since WP3 sorts blocks by hash with the same insertion sort, it is `pack.sort` with each package's files through `K.files_of`, and `blocks_sort` proves the two orders agree. Tools come back as `tools.canon(ts)`: the lock does not write `vendor`, and `M.source_of` reads a missing `root` as `.` and a missing `git` as the hub, which only a tool no ledger could hold differs by. The proof is about 3,800 lines, inside the estimate: 470 in `check/str.bend`, 1,470 in `check/toml.bend` and 1,850 in `lock/PROOF.bend`. One layer the design did not list was needed: that a bare key's first and last chars are not spaces, which takes the ordering of 32-bit words (`Str.word_le_lt`).
...
**Update:** WP5b has landed the `--upgrade` half: `upgrade_idempotent`, `reupgrade_moves_nothing`, `upgrade_settles`, `reupgrade_lays_nothing` and `reupgrade_writes_only_lock` in `lock/LAWS.bend`, relative to EZ-LED-4. Where it differs from the sketch: `after` covers the upgrade (`after.up.laid`): the ledger is the text of the upgrade's ez.toml edit (`ledger.left`), the sources are `W.listing.of`, every tree the lock read, including those the upgrade checked out, is read from BEND_LIB when it was laid, and the upgrade's answers are the same list behind a new answer for `.gitignore`, the text the upgrade left (`ig.left`), which is how "the remote did not change" is stated. `lock_idempotent` and `relock_lays_nothing` now take the premise that the args say no `--upgrade`, which is the World they were about. The upgrade laws' premise is not `P.refuses(w) == False` alone but also `rb`, that ez.toml as `after` holds it parses to `P.ledger.next(w)`: EZ-LED-4's instance, and true as it stands when nothing moved. `upgrade_settles` is the sketch's; beside it, the model the second run renders is the first's, nothing new is laid, and no path but the lock is written, which needs `ML.sync_idempotent` for `.gitignore` and so a ledger whose vendored hashes hold no newline. The key lemma is `kp.same` (and `tk.same` for a tool): a pin at the tip its remote names, whose checkout there agrees with it, is `Keep`. Two behavior changes came out of it, both in `lock/up.bend`'s tools: a tool pin with a rev and no narHash is upgraded as a full pin is and filled where it lands, since filling it at its own rev left a pin the next upgrade moved; and a weighing with no commit or narHash pins nothing, a filled pin records the ref it was filled from as its tag, and a moved tool pin keeps its own tag.

### EZ-RES-4, EZ-RES-5, EZ-RES-6, EZ-RES-8: the upgrade's decisions

**Update:** WP6's first half has landed: EZ-RES-4 and EZ-RES-6 are proved, over `P.ledger.next` and `P.wants.up`, and relative to EZ-LED-4 for ez.toml's bytes. `upgrade_holds_hub` is the sketch below. Where the rest differs from it:

- The frame is two laws, `upgrade_one_frames_deps` (the sketch's `upgrade_one_frames_others`) and `upgrade_one_frames_tools`, over `tool.at`, which is `M.tool.find` over the ledger's tools, because the requirement says every other entry, and a tool is one. Both take the premise `String.is_empty(n) == False{}`: an empty `--package` is no filter (`ez/cmd.bend` passes "" when the flag is absent, and `U.chosen("", _)` chooses everything), so for it the frame is false and the law does not claim it.
- The questions law is `upgrade_one_asks_alone`: on every World whose args say `--upgrade --package n`, `P.wants.up(w)` is exactly the questions `Up.next` asks of `alone(ledger, n)`, the ledger with nothing but the package's own fields and the dependencies and tools named `n`. It replaces `P.resolves.only(P.wants(w), P.url(w, n))`, for two reasons. A URL does not name an entry: two dependencies can share a repository, and a law over URLs would let `--package a` ask about `b`'s tag in the same repository. And the equality is stronger, since it says which refs, tags, commits and ancestry pairs are asked, not only of which remote, while it costs less: an entry `--package` does not choose is held, a held entry adds nothing to the walk's questions, refusal or import swaps (`dkey`, `tkey` in `lock/PROOF.bend`), and the pipeline after the walks reads nothing else. It covers every question kind at once, including `HubHas`, `Ignore` and `Sources`, which are not resolving but are asked of the same entries only.
- Two dependencies or tools may share a name. `--package n` chooses every one of them (`U.chosen` compares names), so `alone` keeps every entry named `n`, and the frames speak of the entry `M.dep` or `M.tool.find` returns for another name, the first of that name.
- `Git.tip.*` and `Git.exact` are read only by the tip a pin moves to, which is EZ-RES-5's and EZ-RES-8's (WP6's second half). EZ-RES-4 and EZ-RES-6 hold whatever the remote printed, so this half adds no `git/LAWS.bend` law.

One thing the proofs showed that no law here depends on: a dependency whose fate is `Halts{""}`, a refusal with an empty reason, is dropped from the walked list without stopping the upgrade, since `pins` reads an empty reason as no refusal. Only an `Answer` of `Miss{""}` leads there, and the interpreter never gives one (every `Miss` it builds starts `ez: `), so it is EZ-TRUST-2's; the frames hold anyway, because the entry a name finds is never one that halted. WP6's second half may want `pins` to count refusals rather than read their text.
...
Each WP ends with the gate passing, bolt at zero errors (`trace` included), its laws tagged, and `SPEC.md` updated for the requirements it proved. WP1 and WP2 are the only ones that change behavior, and each lists its changes in its PR description.
