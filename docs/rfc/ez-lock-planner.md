...
**Update:** WP4 found that a lockable lock could still fail to read back: a file path holding `=` is written as a quoted key, and eztoml v0.1.0's reader cuts the line at that `=` ([eztoml#24](https://github.com/Emerging-Patterns/eztoml/issues/24), closed, fixed in 0.2.x). `ez lock` on a package with a file `a=b.bend` wrote a lock that `ez fetch` read back as the path `a` and refused. `lockable` now also refuses a file path holding `=`, and the planner names the package and the path (`lock/LAWS.bend path_eq_unlockable`, `path_eq_named`). EZ-DOC-1's premise is this `lockable`. Since WP3 the packages are read back in the order of the sorted blocks, so `canon` sorts blocks rather than packages, and the tools come back as `M.tools` reads them, which does not keep `vendor`, so the tools law compares against the tools with `vend` dropped.
