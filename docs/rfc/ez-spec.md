...
| EZ-RES-2 | `ez add` with no entry uses the revision's `[package] entry`, then `[package] bin`, then `main.bend`, and refuses if that file is not in the revision. An add that succeeds prints the import line for that entry's path inside the package, the line `ez publish` prints. | Proved | proved |
