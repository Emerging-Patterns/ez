...
| EZ-HASH-3 | spec_hash (the read side only) | none | Not stated. Nothing relates a vendored tree's directory name to its hash. **Update:** `pkg/walked_named_by_texts` and `pkg/walked_manifest_of_texts` state it over the package walk (A0); the row waits for the commands to lay trees from a plan. **Update:** `add/add_lays_named` states it for every tree `ez add` lays (A2); `ez lock` and `ez fetch` are left. **Update:** `fetch/fetch_lays_named` states it for every tree `ez fetch` lays (WP8); `ez lock` is left. |
...
| EZ-DOC-4 | none | same_rev_keeps (supporting) | No law. **Update:** proved for a plain lock by `lock/lock_idempotent` and `lock/relock_lays_nothing` over the planner (WP5a); the `--upgrade` half is WP5b. **Update:** proved for `--upgrade` by `lock/upgrade_idempotent`, `reupgrade_moves_nothing`, `upgrade_settles`, `reupgrade_lays_nothing` and `reupgrade_writes_only_lock`, relative to EZ-LED-4 (WP5b). |
...
| EZ-RES-1 | is_rev_* (supporting), peeled_agrees | none | No law about tag selection. **Update:** the decisions are proved over what `git ls-remote` printed, and tagged: `choose_release`, `choose_prerelease`, `choose_greatest_release`, `choose_greatest_prerelease`, `choose_is_a_tag` (with `latest_greatest` and `latest_rel_greatest` under them), `branch_of_symref`, `branch_skips_*`, `exact_*` and `is_rev_*`. Pending until `ez add` asks through the planner (A2 of the add design). **Update:** proved in A2, with the plan laws `add_pins_*` and `add_commit_asks_nothing`. |
| EZ-RES-2 | none | none | No law. **Update:** `pkg/absent_entry_refused` states the refusal over the package walk (A0); the precedence and the command wait for A2. **Update:** proved in A2 (`git/entry_pick_*`, `add/add_entry_default`, `add/add_entry_absent`). |
...
| EZ-HASH-3 | partly | `Git.lay` (`git/git.bend:375-380`) writes `lib/<hash>` where `hash` is ez's own `K.pkg.hash(K.pkg_of(entry))`, and writes the manifest from the same file list, so it holds when written. `place` ignores `cp` failures (`git/git.bend:349-355`). Nothing re-checks a tree later: `read.git` checks files against the on-disk manifest but never the manifest against the directory name, and `restore` accepts a cached tree whose manifest text matches without re-reading files (`lock/restore.bend:121-132`). **Update:** `ez fetch` checks every tree it lays, and every tree it keeps, against the name the lock records it under (WP8). |
| EZ-HASH-4 | as trusted | `ez publish` compares its hash to `bend --publish`'s answer (`pub/plan.bend`, `agreed`), but only after bend has uploaded. bend 2.0.25 has no way to report the hash without uploading (`cli_publish` hashes, mines and posts in one step), so this ordering stays. |
...
| EZ-DOC-4 | partly | Holds for the lock bytes when the world is unchanged (output is sorted, the old lock is never read). A repeated `--upgrade` re-clones and re-lays every selected pin and rewrites `.ez/origins.toml`. (**Update:** proved in WP5a and WP5b, see "Phase three progress".) |
...
| ID | Verdict | Evidence |
| :---- | :---- | :---- |
| EZ-TOOL-1 | holds | `ez/tool.bend:141-171`. Empty counts as unset. `mkdir -p` result is ignored; a failed `ln` is reported. **Update (WP9):** proved over `tool/plan.bend`; with nowhere to link, an install now refuses before it builds. |
...
| WP7 | done | EZ-VEN-2 and EZ-VEN-3 proved; EZ-VEN-1's text layer and `ez lock --upgrade` half proved, the row pending on `ez add` (A2). Text: `manifest/LAWS.bend` `sync_allowlist_is_the_ledger`, `sync_keeps_other_lines`, `sync_idempotent`, `vended_once` (EZ-VEN-1); `rewrite_names_new` (EZ-VEN-2); `rewrite_keeps_other_lines`, `rewrite_keeps_line_count`, `rewrite_nothing_named` (EZ-VEN-3); `rewrite_by_line`. Plan: `lock/LAWS.bend` `upgrade_allowlist_is_the_ledger`, `upgrade_allowlist_keeps_other_lines`, `upgrade_vends_the_ledger` (EZ-VEN-1); `upgrade_rewrites_imports` (EZ-VEN-2); `upgrade_leaves_other_lines`, `upgrade_keeps_line_count`, `upgrade_writes_only_hits` (EZ-VEN-3); `upgrade_rewrites_sources`. The key steps: the file `I.sync` writes reads back as the lines `I.lines` made, `I.lines` is idempotent (its output has no `.ez/` line, holds the three rules whenever it holds a hash, and holds the allowlist as one block), a line naming `import <a>/` and `import <b>/` with neither holding `/` has `a` and `b` the same, and every decision `Up.next` makes writes only ez.toml and `.gitignore` (`ok_next`). Behavior changes, each shown on the binary first: the allowlist names each hash once (two vendored dependencies of one tree wrote its line twice); a `.gitignore` of a blank line and a stale allowlist line keeps the blank line (it was written empty); `import <old>` with nothing after the hash is no longer rewritten. `check/str.bend` gained `split_clear`, `clear_append_eq`, `drop_clear`, `and_assoc`, `and_true_left`. |
| WP5b | done | EZ-DOC-4 for `ez lock --upgrade`, relative to EZ-LED-4: `lock/upgrade_idempotent` (run again on the World it leaves, against a remote that did not change, it writes the same lock), `reupgrade_moves_nothing` (it renders the same ledger model), `upgrade_settles` (so it writes no ez.toml), `reupgrade_lays_nothing` (every tree the first run laid is read from BEND_LIB) and `reupgrade_writes_only_lock` (no ez.toml, source or `.gitignore`, for a ledger whose vendored hashes hold no newline, by `ML.sync_idempotent`). `after` now covers the upgrade: the ledger it wrote (`ledger.left`), the sources as it rewrote them, the trees it checked out read from BEND_LIB, and the same remote answers behind `.gitignore` as it left it. `lock_idempotent` and `relock_lays_nothing` take the premise that the args say no `--upgrade`, which is what they were about. The key lemma is `kp.same` (a dependency at its own tip whose checkout agrees is kept) and `tk.same` for a tool; each fate is carried to the next run's (`a.one`, `t.one`), each walk (`w.all`, `tw.all`), then `Up.next` stage by stage (`core`). Found while proving, and shown on the real binary: a tool pin with a rev and no narHash was filled at its rev, and the next `ez lock --upgrade` against the same remote moved it to the default branch tip and rewrote ez.toml and the lock. Such a pin is now upgraded as a full pin is and filled wherever it lands; a weighing that gives no commit or narHash pins nothing; a pin filled from a ref records that ref as its tag; and a tool pin that moves keeps its own tag, as a dependency does. About 4,800 lines of proof in `lock/PROOF.bend`, most of them one lemma per step of the upgrade, generated in the shape of the existing name lemmas. |
...
Under `--upgrade`, measured in WP2 on a copy of this repository whose eztoml is taken from a local clone with its `v0.1.0` tag moved to a commit that changes the package, so `ez lock --upgrade --package eztoml` moves the hash and rewrites 15 sources (median of five runs): 2.09 s wall and 0.91 s user for the binary before WP2, and 2.53 s and 1.22 s for WP2's first cut, which rewrote every source on every round of the demand loop, since a World is planned again from the start each round. With the rewrite moved into the plan, which is made once, it is 2.3 to 2.4 s and 1.0 s. The upgrade's own rounds scan no source (`step` asks the upgrade's questions before it looks at packages), and a plain lock of this repository is unchanged (0.53 s and 0.52 s), so the scanned imports were not moved into the World. Against a real remote the network dominates: `ez lock --upgrade` of this repository took 20.7 s before and 17.1 s after, one run each.
...
| WP | State | Scope |
| :---- | :---- | :---- |
| A0 | done | The walk is done: the spike's walk is in `pkg/pkg.bend` (`K.of`), with a third answer for a file it was not given, and `K.pkg_of` is a loop that answers those from disk, so there is one walk. An import that climbs above the checkout is refused, naming it (`pkg/escaping_import_refused`), and the spike's laws moved to `pkg/LAWS.bend`, tagged EZ-RES-2 and EZ-HASH-3. The other half was decided against in A2: the plan types stay in `lock/plan.bend`, and `ez add` asks `lock/up.bend`'s questions, with one added (`Clone`), rather than a `git/ask.bend` (the design's "Update (A2)"). |
| A1 | done | `ez remove` in planner form (`remove/plan.bend`, `remove/run.bend`): an absent name is refused with exit 1 and nothing written, and a vendored dependency's committed tree is dropped unless another dependency names its hash. Laws in `remove/LAWS.bend`: `remove_refusal_writes_nothing` (EZ-OUT-2), `remove_needs_ledger` (EZ-LED-6), `remove_refuses_unread` (EZ-LED-1), `remove_edits_ledger` (EZ-LED-3), `remove_syncs_allowlist` (EZ-VEN-1), and the untagged `remove_refuses_absent`. `remove_idem` is tagged EZ-LED-3, which is proved; `read_refuses_a_problem` and `render_of_unread_is_blank` are tagged EZ-LED-1. The plan types stay in `lock/plan.bend` (the design's "Update (A1, A4)"). |
| A2 | done | `ez add` in planner form (`add/world.bend`, `add/plan.bend`, `add/run.bend`), asking `lock/up.bend`'s `Tags`, `Head` and `Refs` and a new `Clone`, whose answer is the checkout's narHash and every file's text, walked by `K.of` in the planner. Deleted: `Git.vendor*`, `Git.resolve`, `Git.Got`, `Git.lay`, `Git.note` and the origins table code, `Git.default.ref`, `Git.tags` and `Git.heads`, the add half of `ez/cmd.bend` with `Cmd.ledger.of`; the laws `origin_read`, `but_drops`, `but_keeps` (git) and `ledger_missing_unwritten`, `ledger_present_parsed` (ez). Laws in `add/LAWS.bend`: `add_refusal_writes_nothing` (EZ-OUT-2), `add_needs_ledger`, `add_needs_ledger_asks_nothing` (EZ-LED-6), `add_refuses_unread` (EZ-LED-1), `add_edits_ledger` (EZ-LED-2), `add_syncs_allowlist` (EZ-VEN-1), `add_records_named`, `add_refuses_named` (EZ-LED-7), `add_records_as_given`, `add_path_as_given`, `add_asks_anchored` (EZ-LED-8), `add_pins_chosen`, `add_pins_chosen_rev`, `add_pins_head_rev`, `add_pins_named`, `add_pins_commit`, `add_commit_asks_nothing` (EZ-RES-1), `add_entry_default`, `add_entry_absent` (EZ-RES-2), `add_lays_named` (EZ-HASH-3), and the untagged `add_walk_refused`. Beside them: `entry_pick_entry`, `entry_pick_bin`, `entry_pick_main` in `git/LAWS.bend` (EZ-RES-2), and `lock_needs_ledger` (EZ-LED-6) and `lock_refuses_unread` (EZ-LED-1) in `lock/LAWS.bend`. Behavior changes: a refused add lays nothing, not even in the cache, and leaves no scratch directory under BEND_LIB; no `.ez/origins.toml`; a re-added vendored dependency is laid under `.ez/lib` and its old tree dropped; nothing is asked of the remote before the ledger, the target and a ledger-decided name are checked. |
| A3 | done | Bare `vendor = true` (#73). |
| A4 | done | `ez init` in planner form (`init/plan.bend`, `init/run.bend`), with no behavior change. `init_keeps_ledger` moved from `ez/LAWS.bend` to `init/LAWS.bend` and is a plan law (EZ-LED-6), beside `init_refusal_writes_nothing` (EZ-OUT-2). |
...
| A6 | done | EZ-LED-2 over `Rend.add.keep`: `add_keep_idem` (#73). |
| A7 | done | With A2: EZ-LED-1, EZ-LED-2, EZ-LED-6, EZ-LED-7, EZ-RES-1 and EZ-RES-2 are proved. EZ-LED-8, EZ-HASH-3, EZ-VEN-1 and EZ-OUT-2 stay pending, each with its `ez add` half in, and `SPEC.md` says what is left: the lock's anchoring and `ez fetch` (EZ-LED-8); the trees `ez lock` and `ez fetch` lay (EZ-HASH-3); two pieces of the upgrade half (EZ-VEN-1); `ez fetch`, `ez publish` and `ez tool` (EZ-OUT-2). |

### Fetch progress

| WP | State | Scope |
| :---- | :---- | :---- |
| WP8 | done | `ez fetch` in planner form (`fetch/world.bend`, `fetch/plan.bend`, `fetch/run.bend`), asking the lock's own questions (a tree under BEND_LIB, a hub package) and the upgrade's `Clone`, anchored by the planner; `ez tool` fills a tool's cache through it, from the tool's checkout. Deleted: `lock/restore.bend`, `Say.dep.cached`, `Cmd.lockfile`. Laws in `fetch/LAWS.bend`: `fetch_refusal_writes_nothing` (EZ-OUT-2), `fetch_needs_ledger`, `fetch_needs_ledger_asks_nothing` (EZ-LED-6), `fetch_lays_the_lock` (EZ-FETCH-1), `fetch_lays_named` (EZ-HASH-3), `fetch_asks_anchored`, `fetch_keeps_records` (EZ-LED-8), and the untagged `fetch_needs_lock`, `fetch_needs_lock_asks_nothing`. EZ-FETCH-1 is proved. Behavior changes, each shown on the binary first: a missing ledger or lock is refused, where fetch exited 0; a refusal writes nothing, where a mismatch removed the tree already under BEND_LIB, left a `.work-<hash>` clone and kept the packages laid before it; files that do not hash to their name, and a path that leaves the package, are refused, where they were laid, the second outside BEND_LIB; a tree under BEND_LIB is checked before it is kept; and a tool's relative sources are read from its checkout, not from where `ez tool` ran. `narHash` is still not checked, since each file is checked against its sum and the package against its name. |

### Tool progress

| WP | State | Scope |
| :---- | :---- | :---- |
| WP9 | done | `ez tool run`, `install`, `upgrade` and `sync` in planner form (`tool/world.bend`, `tool/plan.bend`, `tool/run.bend`), asking the upgrade's `Head` and `Clone` and three questions of their own (`Read`, `Exec`, `Locate`), with a plan of their own shape (the checkout laid, the binary made or reused, the program started or the link written) whose writes and end run through the lock's interpreter. Deleted: `ez/tool.bend`, `Say.kept`, `Say.fetching`. Laws in `tool/LAWS.bend`: `tool_refusal_writes_nothing`, `sync_refusal_installs_nothing` (EZ-OUT-2); `bin_dir_ez`, `bin_dir_xdg`, `bin_dir_home`, `bin_dir_none`, `tool_links_in_bin_dir`, `tool_needs_bin_dir` (EZ-TOOL-1); `tool_reuses_record`, `tool_builds_on_miss` (EZ-TOOL-2); `local_rev_dirty`, `local_rev_unchecked`, `local_rev_elsewhere`, `tool_unrevved_builds` (EZ-TOOL-3); `tool_pin_src`, `tool_pin_rev`, `tool_pin_url`, `tool_pin_over`, `tool_free_src`, `tool_free_rev`, `tool_path_rev`, `local_rev_clean` (EZ-TOOL-4); `tool_run_exits_with_program`, `tool_refusal_exits_one` (EZ-TOOL-5); `tool_link_never_runs` (EZ-TOOL-6); `file_pin_bin`, `file_pin_entry`, `file_bin`, `file_entry`, `file_main`, `tool_builds_file`, `out_name_own`, `out_name_app`, `tool_links_named`, `tool_refuses_unsafe_name` (EZ-TOOL-7); `remote_refuses_slug`, `tool_refuses_bad_slug`, `tool_refuses_nowhere` (EZ-TOOL-8); `tool_rest_dash`, `tool_rest_word`, `tool_rest_none`, `tool_run_passes_words`, `run_line_rest` (EZ-TOOL-9). EZ-TOOL-1 and EZ-TOOL-3 to EZ-TOOL-9 are proved; EZ-OUT-2 waits only on `ez publish`. Behavior changes, each shown on the binary first: a refusal writes nothing, where a remote with no ez.toml or a refused package name left its clone and record in the cache and an install with nowhere to link built first; a path that is not the top of its checkout, and a repository that hides untracked files, no longer pass for a clean commit, where both ran stale binaries; binaries and remote checkouts are kept per record and per commit, where a pinned run at another commit rebuilt the binary an install had linked; a package name that is not a TOML bare key is refused, where `../../evil` wrote outside the cache; a pin whose rev is not a commit is refused before anything is asked; `file:///path` is cached as `file://localhost/path`, where it was refused; and `ez tool sync` checks every pin before it installs any. Found and not changed: bolt v1.2.1's own lock records `snap/par.c` under the sum the old `Sha.hex` gave, so `ez tool sync` of this repository stops at the fetch with exit 1 on master and here alike; that is the tool's lock to refresh, not ez's to change. |

### Lock halves

| WP | State | Scope |
| :---- | :---- | :---- |
| WP10 | done | The `ez lock` halves of EZ-LED-8, EZ-HASH-3 and EZ-VEN-1, and the three rows flipped. The lock's World holds the directory it runs in (`W.Args{upgrade, only, here}`), and the planner anchors every question: a package question carries its source anchored (`P.anchor.src`), and each of the upgrade's questions is put to git as `Up.anchor` of the question the upgrade looks its answer up by (`P.Query`); the interpreter no longer calls `Git.anchored`. The lock checks that the files it lays hash to the name, and lays the manifest of those files (`W.body.laid`, `W.manifest`). The lay vocabulary (`P.lays.named`, `P.lay.named`, `P.front`, `P.last`, `P.effects`, `W.weighs`) moved from `add/plan.bend` to the lock, which lays trees for all three commands. Laws in `lock/LAWS.bend`: `lock_lays_named` (EZ-HASH-3); `lock_asks_anchored`, `upgrade_asks_its_questions`, `upgrade_asks_anchored`, `lock_records_as_given`, `upgrade_records_as_given`, `upgrade_records_tools_as_given` (EZ-LED-8); `upgrade_allowlist_kept_in_sync`, `upgrade_unmoved_vends_the_ledger` (EZ-VEN-1). The upgrade ones rest on a walk of `Up.next` (`kp_next` in lock/PROOF.bend) that carries the stop reason, and on per-fate lemmas that each dependency and tool pin keeps its source and a dependency that did not move is the one it was (`st.*`). |
...
`ez publish` refuses a dirty tree (any `git status --porcelain --untracked-files=normal` line) and a package file git does not track, and succeeds only when every `0x` name bend prints is ez's hash (`pub/plan.bend`). Both are proved over its plan (`pub/LAWS.bend`, EZ-PUB-1 and EZ-PUB-2); the comparison runs after the upload, and stays there because bend 2.0.25 cannot report a package's hash without uploading it.

`ez fetch` checks every restored file against the lock: a hub body's digest must start with the lock's sum, a git file's digest must equal it (`lock/restore.bend:33-89`, `hub/hub.bend:16-17`). It never checks `narHash`, and a cached tree whose manifest text matches is trusted without re-reading files. A missing lock is read as empty and fetch exits 0. **Update (WP8):** a cached tree is read and checked like a fetched one, and a missing ledger or lock is refused with exit 1; `narHash` is still not checked (see "Fetch progress").
