...
- [x] `ez add` with a relative path fetches relative to the wrong directory. **Fixed:** `Tgt.anchor` takes a relative path from the directory ez was run in, which is the project's since every command reads `ez.toml` there, and the ledger records the absolute path. git had read it from `.ez/lib/.work-<rev>` for the fetch, and from the enclosing repository's top level for `ls-remote` (`anchor_absolute`, `anchor_relative`).
