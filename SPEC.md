...
| EZ-LED-8 | A path target is recorded in the ledger as it was given, and a relative path resolves against the project root wherever git reads it: `ez add`, `ez lock`, `ez fetch` and `ez lock --upgrade`. | Proved | pending | ez/LAWS.bend anchor_url; ez/LAWS.bend anchor_absolute; ez/LAWS.bend anchor_relative |
