...
| EZ-RES-3 | A target containing `://` or starting `git@` is a git URL. A target starting `/`, `./`, `../` or `~/` is a path. A target of exactly two segments of letters, digits, `-`, `_` and `.`, neither of them `.` or `..`, is `https://github.com/<target>`. Anything else is a path, except the empty word, which is refused. | Proved | proved | ez/LAWS.bend classify_url, classify_scp, classify_abs, classify_here, classify_up, classify_home, classify_github, classify_else |
