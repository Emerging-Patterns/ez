...
| EZ-HASH-3 | When ez writes a package under `<lib>/<h>`, `h` is the 0x hash of the file list whose manifest it writes beside the files. | Proved | pending | pkg/LAWS.bend walked_named_by_texts; pkg/LAWS.bend walked_manifest_of_texts |
...
| ID | Requirement | Level | Status | Law |
| :---- | :---- | :---- | :---- | :---- |
| EZ-LED-1 | A ledger that does not parse is never read into a model, and renders as nothing, so no command writes a guess over it. | Proved | pending | manifest/LAWS.bend read_refuses_a_problem; manifest/LAWS.bend render_of_unread_is_blank; remove/LAWS.bend remove_refuses_unread |
...
| EZ-LED-6 | `ez init` writes nothing when a ledger exists, and `ez add`, `ez remove` and `ez lock`, with or without `--upgrade`, refuse and write nothing when there is none. | Proved | pending | ez/LAWS.bend ledger_missing_unwritten; init/LAWS.bend init_keeps_ledger; remove/LAWS.bend remove_needs_ledger |
...
| EZ-LED-8 | A path target is recorded in the ledger as it was given, with a leading `~/` expanded, and a relative path resolves against the project root wherever git reads it: `ez add`, `ez lock`, `ez fetch` and `ez lock --upgrade`. | Proved | pending | ez/LAWS.bend anchor_url; ez/LAWS.bend anchor_absolute; ez/LAWS.bend anchor_relative |
...
| ID | Requirement | Level | Status | Law |
| :---- | :---- | :---- | :---- | :---- |
| EZ-RES-1 | `ez add` with no ref pins the greatest semver-ish release tag on the remote; with no release, the greatest pre-release; with no semver-ish tag, the remote's default branch as its `HEAD` symref names it. A named ref resolves exactly, as `refs/tags/<ref>` and then `refs/heads/<ref>`. A 40-hex ref is used as a commit without asking the remote. | Proved | pending | git/LAWS.bend is_rev_needs_40; git/LAWS.bend is_rev_needs_hex; git/LAWS.bend is_rev_hex40; git/LAWS.bend choose_release; git/LAWS.bend choose_prerelease; git/LAWS.bend branch_of_symref; git/LAWS.bend branch_skips_other; git/LAWS.bend branch_skips_commit; git/LAWS.bend exact_skips; git/LAWS.bend exact_tag_first; git/LAWS.bend exact_branch; git/LAWS.bend latest_greatest; git/LAWS.bend latest_rel_greatest; git/LAWS.bend choose_greatest_release; git/LAWS.bend choose_greatest_prerelease; git/LAWS.bend choose_is_a_tag |
| EZ-RES-2 | `ez add` with no entry uses the revision's `[package] entry`, then `[package] bin`, then `main.bend`, and refuses if that file is not in the revision. | Proved | pending | pkg/LAWS.bend absent_entry_refused |
...
| EZ-OUT-2 | A command that refuses writes nothing: every file it would otherwise write or remove is left as it found it. | Proved | pending | init/LAWS.bend init_refusal_writes_nothing; lock/LAWS.bend lock_refusal_writes_nothing; remove/LAWS.bend remove_refusal_writes_nothing |
...
| :---- | :---- | :---- |
| EZ-HASH-3 | Over the package walk (`pkg/pkg.bend`): the `0x` name of the files it found, and their manifest, are the name and the manifest of the files it lays, each weighed by its own text. | That every tree a command lays is named by the walk's hash with that manifest beside it (`lays.named` in [docs/rfc/ez-add-planner.md](docs/rfc/ez-add-planner.md)), which needs `ez add`, `ez lock` and `ez fetch` to lay trees from a plan. |
| EZ-LED-1 | Over the ledger model: a document with a problem reads as the problem, and a read that failed renders as nothing. `ez remove`: a ledger that does not parse is refused, so with EZ-OUT-2 it writes nothing (`remove_refuses_unread`). `ez init` never writes over a ledger that is there, whatever it holds (`init_keeps_ledger`, EZ-LED-6). | That `ez add` and `ez lock`, with or without `--upgrade`, refuse a ledger that does not parse, which needs `ez add` in planner form (`add_refuses_unread` in [docs/rfc/ez-add-planner.md](docs/rfc/ez-add-planner.md)) and a law over the lock planner's refusal of an unread ledger. |
...
| EZ-LED-6 | `ez init`: a World with a ledger plans no write at all (`init_keeps_ledger`). `ez remove`: a World with no ledger refuses (`remove_needs_ledger`), so with EZ-OUT-2 it writes nothing. `Cmd.ledger.of` reads a missing ledger, for `ez add`, as a read that renders as nothing. | That `ez add` and `ez lock`, with or without `--upgrade`, refuse a World with no ledger: `ez add` needs planner form (`add_needs_ledger`), and the lock planner, which already refuses one, needs the matching law. |
...
| EZ-RES-1 | The decisions, over what `git ls-remote` printed. `Git.choose` answers one of the tags; a release whenever any tag is one; a release no release is newer than, by `ord.ver`, the order `latest` folds with; and with no release, a tag no semver-ish tag is newer than. `Git.tip.branch` is the branch HEAD's symref row names. `Git.exact` answers `refs/tags/<ref>` (its peeled commit first), else `refs/heads/<ref>`, and no other row. `Git.is_rev` takes exactly forty hex characters as a commit. | That `ez add` pins what these decisions answer over the planner's `Tags`, `Head` and ref answers, falls back to the default branch only when `Git.choose` answers nothing, and asks the remote nothing for a 40-hex ref (`add_pins_chosen` and `add_commit_asks_nothing` in [docs/rfc/ez-add-planner.md](docs/rfc/ez-add-planner.md)), which needs `ez add` in planner form. |
| EZ-RES-2 | Over the package walk: an entry the checkout's files do not hold is refused, naming it, and never hashed as an empty package. | The precedence of the given entry, `[package] entry`, `bin` and `main.bend`, and that `ez add` walks the revision's files from that entry and refuses when the walk does, which needs `ez add` in planner form. |
| EZ-VEN-1 | Over the line-level function `I.lines` (manifest/ignore.bend): its allowlist lines are exactly the vendored hashes, each once, in the order the ledger first names them, and every other line is kept in order. The text layer: the file `I.sync` writes reads back (`I.file.lines`) as the lines `I.lines` made from the file it read, whatever that file ended with, so its allowlist is the vendored hashes and every other line is kept byte for byte, and applying it twice is applying it once; each for a ledger whose vendored hashes hold no newline. `ez remove` leaves `.gitignore` holding exactly `I.sync`'s text (`remove_syncs_allowlist`), so its half is proved. `ez lock --upgrade`: every `.gitignore` its plan writes (`P.put(w, ".gitignore")`) has the allowlist of the hashes the upgrade commits and keeps every other line of the file it read, when no source it read is at `.gitignore`; and when it writes ez.toml those hashes are the vendored hashes of the model it renders (`P.ledger.next`), so the allowlist is that ledger's relative to EZ-LED-4 (below). | The `ez add` half, which calls `I.sync` from interpreter code (EZ-TRUST-2) until it is in planner form (A2). Two pieces of the upgrade half: that an upgrade that leaves `.gitignore` unwritten leaves it because it was already in sync, and that the hashes it commits are ez.toml's own when no pin moved and ez.toml is not written. Both hold of every World on which the upgrade does not refuse, now that every refusal has a reason (`Up.halt.why`), and need the stage-by-stage walk of `Up.next` that `ok_next` in lock/PROOF.bend does, carrying the stop reason as well. |
| EZ-OUT-2 | `ez lock`, plain or `--upgrade`: a World on which the planner refuses has a plan with no write, lay or remove effect, so it writes nothing, not the lock, ez.toml, `.gitignore`, a source or a tree. The upgrade's refusals (a pin off its history, a drift, a hub digest that does not match, an answer IO could not give, an unknown `--package`) and the lock's refusals after an upgrade are all such Worlds. `ez remove` and `ez init`: the same, for every World on which their planners refuse: no ledger, one that does not parse, or a name it does not have, for `ez remove`; a ledger that is there, for `ez init`. | Every other command. `ez add`, `ez fetch`, `ez publish` and `ez tool` write as they go and meet EZ-OUT-2 only when each is converted to planner form. A write the interpreter attempts and the system rejects is EZ-OUT-1's, and what it leaves is EZ-TRUST-2's. |
