...
| ID | Requirement | Level | Status | Law |
| :---- | :---- | :---- | :---- | :---- |
| EZ-VEN-1 | After `ez add`, `ez remove` or `ez lock --upgrade`, the `.gitignore` allowlist names exactly the hashes of dependencies marked `vendor = true`, and every other line of `.gitignore` is unchanged. | Proved | pending | manifest/LAWS.bend allowlist_is_the_ledger; manifest/LAWS.bend allowlist_keeps_other_lines; remove/LAWS.bend remove_syncs_allowlist |
...
| EZ-VEN-1 | Over the line-level function `I.lines` (manifest/ignore.bend): its allowlist lines are exactly the vendored hashes, in ledger order, and every other line is kept in order. | The text layer: `I.sync` splitting `.gitignore` into lines and joining them back, including the trailing newline, so that "every other line is unchanged" holds of the file's bytes. That applying it twice is applying it once. `ez remove` leaves `.gitignore` holding exactly `I.sync`'s text over the dependencies left (`remove_syncs_allowlist`), so its half waits only on that text layer. `ez add` calls `I.sync` from interpreter code, which stays trusted (EZ-TRUST-2) until it is in planner form. `ez lock --upgrade` plans the write of `I.sync`'s text over the ledger it leaves (lock/up.bend), so its half is a law over that plan, `P.put(w, ".gitignore")`, relative to EZ-LED-4 (below). |
